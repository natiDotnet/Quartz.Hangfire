...
public static partial class QuartzJob
{
    ...
    private static Task<TriggerKey> Enqueue(Expression<Action> methodCall)
    {
    ...
    /// <param name="methodCall">The method call expression representing the job to be executed.</param>
    /// <returns>A <see cref="Task{TResult}"/> that represents the asynchronous operation, returning the <see cref="TriggerKey"/> of the enqueued job.</returns>
    private static Task<TriggerKey> Enqueue(string queue, Expression<Action> methodCall)
    {
    ...
    /// <returns>A <see cref="Task{TResult}"/> that represents the asynchronous operation, returning the <see cref="TriggerKey"/> of the enqueued job.</returns>
    private static Task<TriggerKey> Enqueue(Expression<Func<Task>> methodCall)
    {
    ...
    /// <param name="methodCall">The asynchronous method call expression representing the job to be executed.</param>
    /// <returns>A <see cref="Task{TResult}"/> that represents the asynchronous operation, returning the <see cref="TriggerKey"/> of the enqueued job.</returns>
    private static Task<TriggerKey> Enqueue(string queue, Expression<Func<Task>> methodCall)
    {
        return InternalEnqueue(Job.FromExpression(methodCall, queue));
    }
    ...
    /// <param name="methodCall">The method call expression representing the job to be executed.</param>
    /// <returns>A <see cref="Task{TResult}"/> that represents the asynchronous operation, returning the <see cref="TriggerKey"/> of the enqueued job.</returns>
    private static Task<TriggerKey> Enqueue<T>(Expression<Action<T>> methodCall) where T : notnull
    {
        return InternalEnqueue(Job.FromExpression(methodCall));
    }
    ...
    /// <param name="methodCall">The method call expression representing the job to be executed.</param>
    /// <returns>A <see cref="Task{TResult}"/> that represents the asynchronous operation, returning the <see cref="TriggerKey"/> of the enqueued job.</returns>
    private static Task<TriggerKey> Enqueue<T>(string queue, Expression<Action<T>> methodCall) where T : notnull
    {
        return InternalEnqueue(Job.FromExpression(methodCall, queue));
    }
    ...
    /// <param name="methodCall">The asynchronous method call expression representing the job to be executed.</param>
    /// <returns>A <see cref="Task{TResult}"/> that represents the asynchronous operation, returning the <see cref="TriggerKey"/> of the enqueued job.</returns>
    private static Task<TriggerKey> Enqueue<T>(Expression<Func<T, Task>> methodCall) where T : notnull
    {
        return InternalEnqueue(Job.FromExpression(methodCall));
    }
    ...
    /// <param name="methodCall">The asynchronous method call expression representing the job to be executed.</param>
    /// <returns>A <see cref="Task{TResult}"/> that represents the asynchronous operation, returning the <see cref="TriggerKey"/> of the enqueued job.</returns>
    private static Task<TriggerKey> Enqueue<T>(string queue, Expression<Func<T, Task>> methodCall) where T : notnull
    {
        return InternalEnqueue(Job.FromExpression(methodCall, queue));
    }
